...
int obtener_socket_invitado(const char *invitado, ListaConectados *jugadores) {
	   ...
	   {    // Se comprueba que el usuario exista en la BBDD y que la contrasena coincida.
			...
			else if (X == 1)
			{
				if (!UsuarioConectado)
				{
					...
					sprintf(respuesta, "99/%d/%s", JugadoresConectados.num, conectados);
		...
		else if (codigo == 7)
		{
			...
			// Obtener el socket del invitado desde la lista de jugadores conectados
			int socket_invitado = obtener_socket_invitado(invitado, &JugadoresConectados);
			...
			{
				// Si se encuentra el socket, enviar la invitaciÃ³n
				char respuesta[100];
				sprintf(respuesta, "7/%s/%s", invitador, CodigoDeSala);  // Formato del mensaje de invitaciÃ³n
				printf(">> Enviando al socket %d el mensaje: %s\n", socket_invitado, respuesta);
